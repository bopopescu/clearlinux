...
static void do_fuzz(void* data)
{
	...
	while (i-- > 0) {
		...
		if (asprintf(&command, "valgrind --leak-check=no --log-file="
				OUTDIR "valgrind.log.%i ./swupd_patch %s "
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		executor = executor_create(THREAD_COUNT, THREAD_COUNT, true);

		for (i = 1; i <= THREAD_COUNT; i++)
			executor_submit_task(executor, do_fuzz, (void *)(unsigned long)i);
