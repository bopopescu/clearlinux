			   ...
			   size_t *blob_len, xattrs_action_type_t action)
{
	...
	if (len < 0) {
		LOG_ERROR(NULL, "Failed to get x-attribute length", class_xattrs,
				"\\*name=\"%s\",path=\"%s\",strerror=\"%s\"*\\",
	...
	*blob = value;
	...
	if (len < 0) {
		LOG_ERROR(NULL, "Failed to get x-attribute", class_xattrs,
				"\\*name=\"%s\",path=\"%s\",strerror=\"%s\"*\\",
				name, path, strerror(errno));
		return -1;
	}
...
static void xattrs_do_action(xattrs_action_type_t action,
	...
	for (i = 0; i < count; i++) {
		...
		if (action == XATTRS_ACTION_COPY) {
			...
			free(value);
			if (ret < 0) {
				LOG_ERROR(NULL, "Failed to set x-attributes", class_xattrs,
						"\\*dst_filename=\"%s\",strerror=\"%s\"*\\",
