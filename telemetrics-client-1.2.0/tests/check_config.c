...
START_TEST(check_read_valid_config)
{
        ...
        configuration config = { { 0 }, false, NULL };

        int ret = read_config_from_file(config_file, &config);
        ck_assert(ret == true);

        ck_assert_str_eq(config.values[CONF_SERVER_ADDR], "http://127.0.0.1");
        ck_assert_str_eq(config.values[CONF_SOCKET_PATH], "/tmp/test_telem_socket");
        ck_assert_str_eq(config.values[CONF_SPOOL_DIR], "/tmp/spool");
        ck_assert_str_eq(config.values[CONF_RECORD_EXPIRY], "1200");
}
END_TEST
