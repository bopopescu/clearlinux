...
tty_readpass( const char *prompt, char *buf, size_t size )
{
    ...
    if ( size <= 0 )
        return -1;                              /* no room */
    ret_write = write(tty, prompt, strlen(prompt));
    if (ret_write == -1 )
	    fprintf(stderr,"Error to write to tty");
    buf[0] = '\0';
...
void
report_bytes( char *prefix, unsigned char *buf, int len )
{
    if ( ! f_debug )
        return;
    debug( "%s", prefix );
...
int
atomic_out( SOCKET s, unsigned char *buf, int size )
{
    int ret, len;
...
int
atomic_in( SOCKET s, unsigned char *buf, int size )
{
    int ret, len;
